...
class MainView extends StatelessWidget {
          ...
          Container(
            ...
            child: Consumer<MainModel>(
                builder: (context, model, _) => PageView.builder(
                      itemBuilder: (context, position) {
                        return _buildCafeList(context, model.cafes, position + 1, model);
                      },
  ...
  Widget _buildCafeList(BuildContext context,
      UnmodifiableListView<MerchantDetail> itemsList, int page, MainModel model) {
    ...
    var widgets = List.generate(
        itemsList.length,
        (index) =>
            _buildCafeItem(context, itemsList[index + ((page - 1) * constItemsInPage)], model));
  ...
  Widget _buildCafeItem(BuildContext context, MerchantDetail item, MainModel model) {
    return Expanded(
        child: GestureDetector(
          onTap: () {
            model.onCafeItemClick(context, item.merchantId, item.restaurantName);
          },
          child: Card(
          elevation: 8,
